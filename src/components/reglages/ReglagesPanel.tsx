...
function SettingRow({ icon: Icon, label, description, badge, color = "#52525b" }: {
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-4 rounded-2xl transition-all group cursor-default"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3">
          <p className="text-[15px] text-zinc-200" style={{ fontWeight: 450 }}>{label}</p>
          {badge && (
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-3">
      <p className="text-xs font-medium text-zinc-600 uppercase tracking-widest px-1">{title}</p>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="shrink-0 px-7 pt-6 pb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
        <h1 className="text-xl font-semibold text-zinc-100 tracking-tight flex items-center gap-3">
          <Settings size={20} className="text-zinc-400" /> Réglages
        </h1>
        <p className="text-sm text-zinc-600 mt-1">Personnalise DopaTask OS</p>
      </div>
        ...
        <div className="max-w-2xl mx-auto w-full flex flex-col gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl overflow-hidden"
            ...
            <div className="px-5 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
              <p className="text-xs font-medium text-zinc-600 uppercase tracking-wider">Statistiques</p>
            </div>
          ...
          <div className="rounded-2xl px-5 py-4 text-center" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
            ...
            <p className="text-xs text-zinc-700 mt-1">Sync cloud, notifications, profils neuro-cognitifs et plus.</p>
          </div>
