        ...
        <AnimatePresence>
          {adding && (
            <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
              onSubmit={handleAdd} className="mb-5 p-6 rounded-3xl flex flex-col gap-3 bg-surface border border-b-primary"
              ...
              <div className="flex gap-2">
                {HORIZONS.map((h) => (
                  <button key={h.key} type="button" onClick={() => setNewHorizon(h.key)} className="text-[10px] px-3 py-1.5 rounded-lg font-medium transition-all"
                    style={{ background: newHorizon === h.key ? "var(--accent-blue-light)" : "var(--surface)", color: newHorizon === h.key ? "var(--accent-blue)" : "var(--text-primary)", border: `1px solid ${newHorizon === h.key ? "var(--accent-blue-light)" : "var(--border-primary)"}` }}
                  >{h.label}</button>
                ))}
              </div>
            ...
            return (
              <div key={key} className="flex flex-col gap-3">
                ...
                {objs.map((obj) => {
                  ...
                  return (
                    <motion.div key={obj.id} layout className="rounded-3xl overflow-hidden transition-all bg-surface border border-b-primary"
                              ...
                              {obj.milestones.map((ms) => (
                                <div key={ms.id} className="flex items-center gap-2.5 group/ms py-0.5">
                                  <button onClick={() => toggleMilestone(obj.id, ms.id)} className="shrink-0 w-4 h-4 rounded-md border flex items-center justify-center transition-all"
                                    style={{ borderColor: ms.done ? `${obj.color}60` : "var(--border-hover)", background: ms.done ? `${obj.color}15` : "transparent" }}
