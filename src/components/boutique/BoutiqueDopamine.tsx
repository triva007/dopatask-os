...
function RewardCard({ reward, xp, onBuy }: { reward: Reward; xp: number; onBuy: (r: Reward) => void }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col rounded-3xl overflow-hidden transition-all relative bg-surface border-b-primary"
      style={{
      ...
      <div className="px-4 pb-4 flex items-center justify-between">
        ...
        <button
          ...
          style={{
            ...
            color: canAfford ? "var(--accent-blue)" : "#C7C7CC",
          }}
        >
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="shrink-0 px-7 py-4 flex items-center gap-4 bg-surface border-b-b-primary">
        <div className="relative">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 font-bold text-base bg-accent-blue-light text-accent-blue border-b-primary"
          >{level}</div>
          {streak > 0 && (
            <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[8px] font-bold bg-accent-red text-white border-b-primary"
            >
              {streak}
            </div>
          )}
              ...
              {ACHIEVEMENTS.map((a) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <a.icon size={16} style={{ color: unlocked ? a.color : "#C7C7CC" }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium" style={{ color: unlocked ? "var(--text-primary)" : "var(--text-secondary)" }}>{a.name}</p>
                      <p className="text-[12px]" style={{ color: unlocked ? "var(--text-secondary)" : "#C7C7CC" }}>{a.description}</p>
                    </div>
                    {unlocked && (
                  ...
                  <div className="flex items-center justify-between mt-1">
                    ...
                    {dailyChallengeCompleted ? (
                      ...
                      <button
                        ...
                        className="text-xs px-4 py-2 rounded-xl font-medium transition-all bg-accent-blue-light border-b-primary text-accent-blue"
                      >Valider</button>
                    )}
                  </div>
                </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-2 px-5 py-3 rounded-2xl text-sm font-medium z-50 bg-surface border-b-primary text-accent-green"
