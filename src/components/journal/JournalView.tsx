      ...
      <div className="shrink-0 px-7 pt-6 pb-4 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
        <div>
          <h1 className="text-lg font-semibold text-zinc-100 tracking-tight flex items-center gap-2.5">
            <BookOpen size={18} className="text-zinc-400" /> Journal
          </h1>
          <p className="text-xs text-zinc-600 mt-1">Vide-toi la tête · {journalEntries.length} entrées</p>
        </div>
        ...
        <div className="shrink-0 px-7 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.04)" }}>
            ...
            <input
              ...
              className="flex-1 text-sm bg-transparent text-zinc-300 placeholder:text-zinc-700 focus:outline-none"
            />
            {searchQuery && (
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-6 py-5 flex flex-col gap-5">
          ...
          {composing && (
            ...
            >
              <textarea
                ...
                rows={5}
                autoFocus
                className="text-sm bg-transparent text-zinc-200 placeholder:text-zinc-600 focus:outline-none resize-none leading-relaxed"
              />

              {/* Mood picker */}
              <div className="flex items-center gap-3">
                <span className="text-[10px] text-zinc-600 uppercase tracking-widest font-medium">Humeur</span>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => { setComposing(false); setContent(""); setMood(undefined); }} className="text-xs text-zinc-600 hover:text-zinc-400 px-3 py-1.5">Annuler</button>
                <button
        ...
        {grouped.map((group, gi) => (
          <div key={gi} className="flex flex-col gap-3">
            <p className="text-[10px] font-medium text-zinc-600 uppercase tracking-widest px-1">{group.label}</p>
                    ...
                    <div className="flex-1 min-w-0">
                      {isEditing ? (
                        <textarea
                          ...
                          autoFocus
                          rows={4}
                          className="w-full text-sm bg-transparent text-zinc-200 focus:outline-none resize-none leading-relaxed"
                        />
                      ) : (
                        <p
                          className="text-sm text-zinc-300 leading-relaxed whitespace-pre-wrap cursor-pointer"
        ...
        {journalEntries.length === 0 && !composing && (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <div className="text-center">
              <p className="text-sm text-zinc-400">Ton journal est vide</p>
              <p className="text-xs text-zinc-600 mt-1">Écris ce qui se passe dans ta journée. C&apos;est ton espace, sans jugement.</p>
            </div>
            <button onClick={() => setComposing(true)} className="text-xs px-4 py-2 rounded-xl font-medium transition-all"
