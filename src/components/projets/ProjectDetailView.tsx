...
export default function ProjectDetailView({ project, onBack }: { project: Project; onBack: () => void }) {
  // Use optional chaining and memoization for safety
  ...
  const linkedGoogleTasks = useMemo(() => (googleTasks || []).filter(t => {
    if (!t?.id || !project?.id) return false;
    // Check both normal and prefixed IDs
  ...
  const linkedGoogleEvents = useMemo(() => (googleEvents || []).filter(e => 
    e?.id && project?.id && googleEventProjects[e.id] === project.id
  ...
  if (!project) return (
    <div className="p-8 text-center bg-[var(--card-bg)] h-full">
      ...
      <button onClick={onBack} className="mt-4 text-[var(--accent-blue)] underline">Retour aux projets</button>
    </div>
  );
  ...
  try {
                    ...
                    {activeLocalTasks.map(task => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[14.5px] font-medium text-[var(--text-primary)] break-words">{task?.text || "Sans titre"}</p>
                          {task?.description && <p className="text-[12px] text-[var(--text-tertiary)] mt-1 line-clamp-1 break-words">{task.description}</p>}
                        </div>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="space-y-2 opacity-50">
                      {completedLocalTasks.map(task => (
                        <div key={task?.id || Math.random().toString()} className="flex items-start gap-3 p-3 rounded-xl border border-[var(--border-primary)] bg-[var(--surface-2)]">
                ...
                {loadingGoogle ? (
                  ...
                  <div className="space-y-2.5">
                    {linkedGoogleTasks.map(task => (
                      <div key={task?.id || Math.random().toString()} className="flex items-start gap-3 p-4 rounded-2xl border border-[var(--border-primary)] bg-[var(--surface-1)] shadow-sm">
                        <div className="mt-1 w-4 h-4 rounded-full border-2 border-[var(--accent-cyan)] opacity-50" />
                        <div className="flex-1 min-w-0">
                          <p className="text-[14.5px] font-medium text-[var(--text-primary)] break-words">{task?.title || "(sans titre)"}</p>
                          {task?.notes && <p className="text-[12px] text-[var(--text-tertiary)] mt-1 line-clamp-1 break-words">{task.notes}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {loadingGoogle ? (
                  ...
                  <div className="space-y-3">
                    {linkedGoogleEvents.map(event => {
                      ...
                      return (
                        <div key={event?.id || Math.random().toString()} className="p-4 rounded-2xl border border-[var(--border-primary)] bg-[var(--surface-1)] shadow-sm">
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-[14.5px] font-bold text-[var(--text-primary)] break-words">{event?.summary || "(sans titre)"}</p>
                          ...
                          <div className="flex items-center gap-2 text-[12px] text-[var(--text-secondary)]">
                            ...
                            {start && !isNaN(start.getTime()) ? start.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }) : "Toute la journée"}
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {notes.map(note => {
                      ...
                      return (
                        ...
                        >
                          <h3 className="text-[15px] font-bold text-[var(--text-primary)] mb-2.5 break-words">{note?.title || "Note sans titre"}</h3>
                          <p className="text-[13px] text-[var(--text-secondary)] line-clamp-4 leading-relaxed break-words">{note?.content || ""}</p>
    ...
    console.error("ProjectDetailView Critical Render Error:", err);
    return (
      <div className="p-8 text-center">
        <p className="text-red-500 font-bold">Erreur d'affichage</p>
        <p className="text-xs text-gray-500 mt-2">{String(err)}</p>
        <button onClick={onBack} className="mt-4 underline">Retour</button>
      </div>
    );
  }
}
